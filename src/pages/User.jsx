...
export const User = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            axios.get('http://localhost:4080/auth/all').then((response) => {
        ...
        axios.get(`http://localhost:4080/auth/${formData.id}`)
            .then((response) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:4080/auth/signuppetugas', {
                method: 'POST',
                headers: {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`http://localhost:4080/auth/${editFormData._id}`, JSON.stringify(editFormData), {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
    ...
    const handleDelete = (itemId) => {
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete(`http://localhost:4080/auth/${itemId}`, {
                        headers: {
                            Authorization: `Bearer ${token}`,
                    ...
                    {dataUser ? (
                        dataUser.filter((user) => !selectedRole || user.role === selectedRole).map((User) => (
                            <div key={User._id} className='flex w-full h-[100px] mt-5 bg-white shadow rounded-lg'>
                                ...
                                <span className="mx-auto my-auto w-[500px]">
                                    <div className={`h-8 w-28 flex my-auto rounded-xl ${User.role == "Infrastruktur" ? "bg-Gold" : User.role === "Sosial" ? "bg-blue-600" : User.role === "Admin" ? "bg-Hijau-tua" : "bg-Gold"}`}>
